...
const SOFT_SKILL_LINE_PATTERNS = [[], [8, 4], [4, 4], [2, 3], [12, 4]];

export default function SoftSkillsTrendChart({
  filters,
  ...
  // true = garis tersambung
  // false = garis putus saat data kosong
      ...
      .then(([trendRes, totalJobsRes]) => {
        ...
        monthlyData.forEach((monthItem) => {
          if (monthItem.skills && Array.isArray(monthItem.skills)) {
            monthItem.skills
              .filter((skill) => skill.skill_type === "soft_skill")
              .forEach((skill) => {
        ...
        const skillsData = orderedTopSkills
          .map((skill, idx) => {
            ...
            const color = SOFT_SKILL_COLORS[idx % SOFT_SKILL_COLORS.length];
  ...
  return (
    ...
    >
      ...
      <Text fontSize="sm" color="var(--text-secondary)" mb={2}>
        Tren permintaan kemampuan interpersonal dan leadership
      </Text>
