...
export default function TechnicalSkillsTrendChart({
  filters,
  ...
  useEffect(() => {
    ...
    const params = buildQueryString({
      ...
      month: filters?.month,
      year: filters?.year,
      skill_type_id: 2, // 2 = technical_skill
      limit: 20,
    });

    const totalJobsParams = buildQueryString({
      skill_type_id: 2, // Technical skill type
      keyword_id: filters?.keyword_id,
      location: filters?.location,
      employee_size: filters?.employee_size,
      month: filters?.month,
      ...
      .then(([trendRes, totalJobsRes]) => {
        ...
        monthlyData.forEach((monthItem) => {
          if (monthItem.skills && Array.isArray(monthItem.skills)) {
            monthItem.skills
              .filter((skill) => skill.skill_type === "technical_skill")
              .forEach((skill) => {
        ...
        const skillsData = orderedTopSkills
          .map((skill, idx) => {
            ...
            const color =
              TECHNICAL_SKILL_COLORS[idx % TECHNICAL_SKILL_COLORS.length];
  ...
  return (
    ...
    >
      <Heading size="md" color="var(--text-primary)" mb={2} fontWeight="bold">
        Technical Skills Trends
      </Heading>

      <Text fontSize="sm" color="var(--text-secondary)" mb={2}>
        Tren permintaan kemampuan teknis spesifik
      </Text>
